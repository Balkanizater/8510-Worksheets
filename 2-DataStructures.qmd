...
>Line 48 excluded positions 1 and 3 due to the negative integer and the brackets outside, which indicate (these are indices).
...
>I think it shows only values that are above the TRUE value.
...
```{r}
matrix((2+2)=18), byrow=TRUE, nrow=3)
```
...
>the standard format is [rows, columns], and if you want just rows, do this [rows,] and just columns is [, columns]
...
>I think that it limits the dataset to one column, cities, and rows 100 to 110.
...
```{r}
x <- 25
 if (x < 30) print("Space available") else if (x > 30) print("Class Full")
```
...
```{r}
#Presidents = list(Jefferson, Washington, Adams, Monroe) print(Presidents) if list < 5
Presidents <- c("Jefferson", "Adams", "Washington", "Monroe") 
if (length(Presidents) > 5) print("There are more than five") else print("There are fewer than five")
```
...
if (max(gayguides$Year) > 1990) {
  print("There is a year later than 1990")
  } else {
    print("There is no year later than 1990")}
```
...
> This function works because we first set up what "presidents" containts and then we use the length function along with 1: to indicate the series to print.
...
```{r}
classes <- c("History 8500", "History Independent Study")
for (p in 1:length(classes)) {print(classes[p])}
```
...
Because of the limits on i less than 3 and the if i is >= 5 then we exit the program.
...
```{r}
for (s in 1:length(statepopulations$X1800)) {
  if (is.na(statepopulations$X1800[s])) {
    next
  }
  else if (statepopulations$X1800[s] < 200000) {
    next 
    }
   else if (statepopulations$X1800[s] > 250000) {
      print(statepopulations$STATE[s])
  }
}
...
for (x in 1:length(gayguides$city)) {
  if x <- "Greenville" print("Greenville") 
}
```
...
```{r}
 hic <- nchar("History is Cool") 
  print(paste("There are", hic, "in that there string"))
```

(@) Reflect on the process of building the above function. How did you go about figuring this out? 

>I do not know why it did not work. Though it did turn in the right number of characters.
