...
library(DigitalMethodsData)
data("UndergroundRR")
geo.underground <- undergroundRR %>% geocode(city = City, state = State, method='osm', lat = latitude, long = longitude)
head(geo.underground)
...
BWV <- BostonWomenVoters[1:50,]
BWV.2 <- BWV %>% mutate(location = paste0(Street.Number.on.April.1, " ", Street.of.Residence.on.April.1, "," , "Boston, MA"))


BWV.Coord. <- BWV.2 %>% geocode(location, method = 'osm', lat = latitude, long = longitude) 
```
...
```{r}
BostonWomenVoters %>% qmplot(Occupation, Volume)
```
...
>map_data() can upload all sorts of maps, from various continents, states countries. It is part of the ggplot2 package.
...
> US map showing the various states. It tells us about all geographic enitites by providing latitude and longitude information.
...
```{r}
ContinentalUS <- usa %>% filter(region != "Alaska Territory" | region != "Hawaii Territory")
continentalGG <- gayguides %>% filter(state != "AK") %>% filter(state != "HI")
ggplot() + 
  geom_map( data = ContinentalUS, map = ContinentalUS, aes(long, lat, map_id=region), fill = "magenta") +
  geom_point(data = continentalGG, mapping = aes(x=lon, y=lat)) +
labs(title = "Gayguides Continental USA")
```
```
...
library(ggplot2)
library(maps)
SCmap <- map_data("state", state = "SC")
SCdata <- gayguides %>%
  filter(state == "SC")
ggplot() +
  geom_polygon(data = SCmap, aes(x = long, y = lat, group = group), fill = "purple", color = "white") +
  geom_point(data = SCdata, aes(x = lon, y = lat), color = "lime green", size = 0.25) +
  ggtitle("GG Locations in South Carolina") +
  theme_minimal()
```
...
BWV.Coord. <- BWV.2 %>% geocode(location, method = 'osm', lat = latitude, long = longitude) 
ggplot() + 
  geom_map( data = , map = usa, aes(long, lat, map_id=region), fill = "magenta") +
  geom_point(data = gayguides, mapping = aes(x=lon, y=lat))
```
...
>The map is easier to read since it does not have to include those two geographically separated states.
...
```{r}
nhgis <- nhgis %>%
  mutate(percentageforeignbornmales = (AZF003 / total_male_pop) * 100)
```
...
Scale Fill Continuous modifies the "gradient scale of continuous fill aesthetics in a plot."

Now create the map: 
```{r}
library(ggplot2)
library(scales)
ggplot(data = nhgis, aes(fill = percentageforeignbornmales)) +
  geom_sf() +
  scale_fill_continuous("", labels = percent) +
  ggtitle("Percentage: Foreign Born Males") +
  theme_minimal()
```
...
data("gayguides")
colnames(gayguides)
gayguides1 <- gayguides %>%
  filter(!is.na(lon) & !is.na(lat) & lon != "" & lat != "" & !is.na(city))
 GG_map <- leaflet(data = gayguides) %>%
  addTiles() %>%
  addMarkers(
    lng = ~lon,
    lat = ~lat,
...
>ggplot is better for more precise mapping, leaflet is for quick, less accurate mapping.
...
```{r}
library(ggplot2)
read.csv("https://raw.githubusercontent.com/regan008/DigitalMethodsData/main/raw/parks-geocoded.csv")
...
ggplot(parks, aes(x = state_region_province, y = distance_from_closest_city)) +
  ...
  labs(x = "State/Region/Province", y = "Distance to Nearest Urban Area (km)",
       title = "Park Proximity to Urban Areas by Region")
```
```{r}
```{r}
```{r}
