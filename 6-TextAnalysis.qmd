...
```{r}
sotu_texts <- readtext("sotu_files/")
```
...
```{r}
tidy_sotusentences <- sotu_whole %>% unnest_tokens(sentence, text, "sentences")
tidy_sotusentences 
```
...
>#We have arranged sotu data to be readable and to have stopwords inserted in it. Basically in one program we redid everything we had done in the previous parts of the worksheet.
...
>Probably because our attention spans are shorter in general, and probably in the case of the previous President it reflects his particularly low level of concentration. However, Trump is actually around the middle in terms of SOTU length, Presidents Washington and Adams are the shortest.

(@) Filter the dataset to address this discrepancy and the recreate these statistics: 
```{r}
average.word <- tidy_sotu %>% group_by(sotu_type, doc_id, president) %>% 
  count(doc_id)  %>% 
  group_by(sotu_type,  president) %>% 
  summarize(avg_words = mean(n)) %>% 
  arrange(desc(avg_words))
```
...
> So, what is going on here is that we are analyzing now often the words are used, by President, by the total number of words by doc.
...
(@) Pick a president who served more than one term. Filter the dataset and generate both raw word counts and tf-idf scores for that president. What words are most significant in each method? Why and what does that tell you about that president? #Empower comes out as the most significant word, when using tf-idf.


```{r}
wbush.count <- tidy_sotu %>% 
  filter(president == "George W. Bush") %>% 
  count(word, sort= TRUE)

wbush.tfidf <- tidy_sotu %>% 
  filter(president == "George W. Bush") %>% 
...
>My plan is to segment out promotion vs. research topics and find different words in each.
...
```{r}
library(dplyr)
bb.metada.test <- bb.metadata %>% separate(Date, c("Month", "Day", "Year"))

dates.incorrect <- bb.metada.test %>% filter(Day == "May" | Day == "Jul" | Day == "Jun")
bad.months <- c("May", "Jul", "Jun")
dates.correct <- bb.metada.test %>% filter(!Day %in% bad.months)
dates.incorrect <- dates.incorrect %>% rename("month" = "Day")
dates.incorrect <- dates.incorrect %>% rename("day" = "Month")
dates.incorrect <- dates.incorrect %>% rename("Day" = "day") %>% rename("Month" = "month")

bb.metadata <- rbind(dates.correct, dates.incorrect)
```
...
bb.whole <- 
 # sort metadata
  bind_cols(bb.metadata, bb) %>% # combine with texts
  as_tibble() # co
...
```{r}
bb.filter.p <- bb_whole_tidy %>% filter(R_P_E == "P")
#so now we have filtered out P.
```


```{r}
bb_tf_idf <- bb_whole_tidy %>% count(Filename, word, sort = TRUE) %>% 
  bind_tf_idf(word, Filename, n)

bb_tf_idf %>% arrange (desc(tf_idf))
...
I focused on the promotion items in your dataset. In order to make the dataset readable, I had to make all dates conform to one format. After this correction, I started to focus on all of the "P" (Promotion) items.

I used the tf_idf function to create a table of all of the promotion items. This analysis will show term frequency for a given word, along with the how common or rare it is across documents. I think that this information, in the hands of a subject matter expert with contextual knowledge of the corpus, will be very useful.
