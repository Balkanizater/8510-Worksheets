...
> The data compares life expectancy and per capital GDP which is indeed often compared.
...
```{r}
#ggplot(data = gapminder, mapping = aes(x = population, y=lifeExp))
```
...
```{r}
#ggplot(data = gapminder, mapping = aes(x = gdpPercap, y=lifeExp, color=year)) + geom_point() + geom_point (trend)+ labs(x="GDP Per Capita", y="Life Expectency in Years", title="Life Expectency and Per Capita GDP by Year") 
```
...
religious_institutions_over_time <- gayguides %>%
  filter(str_detect(description, "church|mosque|synagogue|temple") | str_detect(type, "Religious Institution")) %>% 
group_by(Year) %>%
  summarize(number_of_institutions = n()) 

ggplot(religious_institutions_over_time, aes(x = Year, y = number_of_institutions)) + 
  geom_line() +
  geom_point() + labs(x = "Year", y = "Number of Religious Institutions", title = "Number of Religious Institutions Over Time") +
  theme_minimal()
```
...
```{r}
#ggplot(rec.workers, aes(x = city, y = year(x1945))) +
 # geom_line() + facet_wrap(~State)
```
...
```{r}
#data("gayguides" )
#gg.byyear <- gayguides %>% filter(state == "SC" | state == "NC" | state == "GA") %>% group_by(state, Year ==1965:1980) %>% summarize(count = n())
...
```{r}
 data("gayguides" )
gg.byyear <- gayguides %>% filter(state == "UT" | state == "ID" | state == "AZ") %>% group_by(state, Year) %>% summarize(count = n())
...
(@) Plot 1 explanation: 
>I was trying to look at similar data for the intermountain west (UT, ID, AZ). Same upward trajectory as in SC, GA, NC.
...
library(DigitalMethodsData)
data("gayguides")
Utah <- gayguides %>% filter(state == "UT") %>% filter(str_detect(amenityfeatures, '(AYOR) | (HOT)'))
```

(@) Plot 2 explanation: 
>Sometimes the lack of information is information itself. There are no locations in this category.
...
```{r}
ggplot( data=rec.workers, aes(x=X1930)) + 
...
(@) Plot 3 explanation: 
>I pulled the code from a ggplot2 site and chose a histogram. I can see this as being particularly useful in my research on cotton merchants.
